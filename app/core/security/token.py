...
def generate_access_token(payload: dict, expiry: Optional[int] = None) -> TokenData:
    if expiry is None:
...
def verify_access_token(token: str) -> Optional[dict]:
    try:
        decoded_jwt = jwt.decode(token, settings.token_secret_key, algorithms=[settings.token_algorithm], options={"verify_aud": False})
        return decoded_jwt
